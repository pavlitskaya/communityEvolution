...
bool bfs(T_DV<pair_t>& d_pairs, T_DV<uint8_t>& propinquities, uint32_t minimum, snapshot_t& communities){
	...
	uint32_t n = get_number_of_diff_elements(
		T_DV<uint32_t>(T_MTI(d_pairs_mirror.begin(), first_element()), T_MTI(d_pairs_mirror.end(), first_element())));
...
bool handle_increment(T_DV<pair_t>& all_pairs){
	...
	uint32_t offset = 0;
	do{
		uint32_t allocate = min(CPULIMIT, d_unique_pairs.size() - offset);
...
T_DV<pair_t> get_specific_pairs(T_DV<pair_t> &d_pairs, T_DV<uint32_t> &d_firsts, T_DV<uint32_t> &d_degree, T_DV<uint32_t> &d_id){
	...
	T_TRYCATCH(
		...
		thrust::copy_if(T_MCI<uint32_t>(i), T_MCI<uint32_t>(min(i + 50000, d_pairs.size())), d_indices.begin(), get_indices(RAWD(d_firsts_red), RAWD(d_degree_red), d_firsts_red.size()));
		})
...
bool calculate_propinquity(T_HV<pair_t>& h_pairs){
	...
	get_degree_mirror(T_DV<uint32_t>(T_MTI(d_pairs_mirror.begin(), first_element()), T_MTI(d_pairs_mirror.end(), first_element())), d_degree);
	get_firsts(d_degree, d_firsts);
	...
	get_intersection(T_DV<pair_t>(h_pairs.begin(), h_pairs.end()), T_DV<pair_t>(d_pairs_mirror.begin(), d_pairs_mirror.end()), d_intersection, d_degree);
...
bool algorithm_propinquity(comevo::Source &source, comevo::Source &target, uint32_t from, uint32_t to, Threshold &threshold, uint32_t bfsMinimum, uint32_t maxIterations, uint32_t maxSnap){
	...
	for (U32 snapId = 0; snapId < nSnaps; ++snapId){
		...
		communities.clear();
		if (!h_pairs.empty())
			bfs(T_DV<pair_t>(h_pairs.begin(), h_pairs.end()), T_DV<uint8_t>(0), bfsMinimum, communities);
		snaps.push_back(communities);
	}
