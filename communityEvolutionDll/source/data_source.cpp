...
namespace comevo{
	...
	bool Source::store_in_file(string filename, bool binary){
		...
		ofstream ofs;
		if(fileType == PAIRS)
			ofs = ofstream("../storage/pairs/" + filename, ios::binary);
		if (fileType == SNAPS)
			ofs = ofstream("../storage/snaps/" + filename, ios::binary);
		ofs.close();
	...
	nodes_t Source::get_nodes(uint32_t index){
		nodes_t nodes;
		pairsToUniqueNodes(get_edges(index), nodes);
		return nodes;
	}
	...
	void Source::display(){
		...
		if (fileType == SNAPS){
			display_snapshots(get_snaps(), "snaps");
		}
	}
